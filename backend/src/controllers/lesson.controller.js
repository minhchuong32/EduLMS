...
const getLesson = async (req, res) => {
  try {
    ...
    if (!result.recordset.length) return res.status(404).json({ error: 'Lesson not found' });
...
const updateLesson = async (req, res) => {
  try {
    ...
    const fileUrl = req.file ? `/uploads/lessons/${req.file.filename}` : undefined;
...
const addComment = async (req, res) => {
  try {
    ...
    `, { lessonId: id, authorId: req.user.id, content, parentId: parentId || null });
...
module.exports = { getLessonsByCourse, getLesson, createLesson, updateLesson, publishLesson, deleteLesson, addComment };
