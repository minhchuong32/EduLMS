...
import { ClockIcon, CheckCircleIcon, ChevronUpIcon } from '@heroicons/react/24/outline';
...
export default function QuizPage() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await submissionApi.submitQuiz(submission.id, formattedAnswers);
      setResult(data);
      setSubmitted(true);
    } catch (err) {
          ...
          {result.score !== undefined ? (
            <>
              ...
              <div className="bg-blue-50 rounded-2xl p-6 mb-6">
                ...
                <p className="text-gray-500 mt-1">/ {result.totalPoints} điểm</p>
                <p className="text-sm text-gray-400 mt-1">{Math.round((result.score / result.totalPoints) * 100)}%</p>
              </div>
            </>
          ) : (
            <p className="text-gray-500 mb-6">Kết quả sẽ được công bố sau.</p>
          )}
          <button onClick={() => navigate(-2)} className="w-full bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700">
            Quay về
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex-1 min-w-0">
          <h1 className="font-bold text-gray-800 text-sm md:text-base truncate">{assignment?.title}</h1>
          <p className="text-xs text-gray-500">Câu {currentQ + 1}/{questions.length}</p>
        </div>
        {timeLeft !== null && (
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-mono font-bold text-sm ml-2
      ...
      <div className="max-w-3xl mx-auto p-4 md:p-6 md:flex md:gap-6">
        ...
        <div className="hidden md:block w-44 flex-shrink-0">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sticky top-24">
                ...
                <button key={i} onClick={() => setCurrentQ(i)}
                  className={`w-8 h-8 rounded-lg text-xs font-medium transition-all
                    ${i === currentQ ? 'bg-blue-600 text-white' :
                      answers[q.id]?.length > 0 ? 'bg-green-100 text-green-700' :
            ...
            <div className="mt-3 pt-3 border-t border-gray-100 text-xs text-gray-500 space-y-1">
              <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-green-100 rounded" /> Đã làm ({answeredCount})</div>
              <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-gray-100 rounded" /> Chưa làm ({questions.length - answeredCount})</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1">
          {question && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6 mb-4">
              <div className="flex items-start gap-3 mb-5">
                ...
                <div>
                  <p className="font-medium text-gray-800 leading-relaxed text-sm md:text-base">{question.questionText}</p>
                  <p className="text-xs text-gray-400 mt-1">
                    {question.questionType === 'single_choice' ? 'Chọn một đáp án' :
                     question.questionType === 'multiple_choice' ? 'Chọn nhiều đáp án' : 'Đúng hay Sai'}
              ...
              <div className="space-y-2.5">
                {question.options.map(opt => {
                  ...
                  return (
                    <button key={opt.id}
                      onClick={() => selectOption(question.id, opt.id, question.questionType)}
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            {currentQ < questions.length - 1 ? (
              ...
              <button onClick={() => window.confirm(`Nộp bài? Đã làm ${answeredCount}/${questions.length} câu.`) && handleSubmit()}
                disabled={submitting}
      ...
      {showNavigator && (
        <div className="md:hidden fixed inset-0 bg-black/40 z-40 flex items-end" onClick={() => setShowNavigator(false)}>
          <div className="bg-white rounded-t-2xl w-full p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-4 text-xs text-gray-500 mb-4">
              <span className="flex items-center gap-1.5"><div className="w-3 h-3 bg-green-100 rounded" /> Đã làm ({answeredCount})</span>
              <span className="flex items-center gap-1.5"><div className="w-3 h-3 bg-gray-100 rounded" /> Chưa làm ({questions.length - answeredCount})</span>
            </div>
            <button
              onClick={() => { setShowNavigator(false); window.confirm(`Nộp bài? Đã làm ${answeredCount}/${questions.length} câu.`) && handleSubmit(); }}
              disabled={submitting}
