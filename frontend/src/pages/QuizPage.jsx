      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          {result.score !== undefined ? (
            ...
            <p className="text-gray-500 mb-6">Bài làm của bạn đã được ghi nhận. Kết quả sẽ được công bố sau.</p>
          )}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div>
          <h1 className="font-bold text-gray-800">{assignment?.title}</h1>
          <p className="text-sm text-gray-500">Câu {currentQ + 1}/{questions.length}</p>
        </div>
        {timeLeft !== null && (
          <div className={`flex items-center gap-2 px-4 py-2 rounded-xl font-mono font-bold text-lg ${timeLeft < 60 ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>
        ...
        <div className="w-48 flex-shrink-0">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sticky top-24">
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 text-xs text-gray-500 space-y-1">
              <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-green-100 rounded"></div> Đã trả lời ({Object.keys(answers).length})</div>
              <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-gray-100 rounded"></div> Chưa làm ({questions.length - Object.keys(answers).length})</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1">
          {question && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-4">
              <div className="flex items-start gap-3 mb-6">
                ...
                <div>
                  <p className="font-medium text-gray-800 leading-relaxed">{question.questionText}</p>
                  <p className="text-xs text-gray-400 mt-1">
                    {question.questionType === 'single_choice' ? 'Chọn một đáp án' :
                     question.questionType === 'multiple_choice' ? 'Chọn nhiều đáp án' : 'Đúng hay Sai'} • {question.points} điểm
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                {question.options.map(opt => {
                  ...
                  return (
                    <button key={opt.id} onClick={() => selectOption(question.id, opt.id, question.questionType)}
          ...
          <div className="flex items-center justify-between">
            ...
            {currentQ < questions.length - 1 ? (
              ...
              <button onClick={() => window.confirm('Nộp bài thi?') && handleSubmit()}
                disabled={submitting}
