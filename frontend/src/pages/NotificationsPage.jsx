    ...
    <div className="max-w-2xl mx-auto p-4 md:p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          {unread > 0 && <p className="text-sm text-blue-600 mt-0.5">{unread} chưa đọc</p>}
        </div>
        {unread > 0 && (
          ...
          {notifications.map(n => (
            <div key={n.id}
              ...
              <div className={`w-9 h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center flex-shrink-0
                ...
                <BellIcon className={`w-4 h-4 md:w-5 md:h-5 ${n.isRead ? 'text-gray-400' : 'text-white'}`} />
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium leading-snug ${n.isRead ? 'text-gray-700' : 'text-gray-900'}`}>
                  {n.title}
                </p>
                {n.message && (
                  <p className="text-xs md:text-sm text-gray-500 mt-0.5 line-clamp-2">{n.message}</p>
                )}
