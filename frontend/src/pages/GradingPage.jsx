...
export default function GradingPage() {
  ...
  useEffect(() => {
    Promise.all([assignmentApi.getById(id), submissionApi.getByAssignment(id)])
      .then(([a, s]) => { setAssignment(a.data); setSubmissions(s.data); })
  ...
  const handleGrade = async () => {
    ...
    try {
      ...
      setSubmissions(prev => prev.map(s =>
        s.id === selected ? { ...s, score: parseFloat(gradeForm.score), status: 'graded' } : s
      ));
    } catch {
  ...
  return (
    <div className="max-w-7xl mx-auto p-3 md:p-6">
      <h1 className="text-lg md:text-xl font-bold text-gray-900 mb-0.5 truncate">
        {!showDetail || window.innerWidth >= 768 ? `Bài nộp: ${assignment?.title}` : (
          <button onClick={() => setShowDetail(false)} className="flex items-center gap-1 text-blue-600 text-base font-semibold">
            <ChevronLeftIcon className="w-4 h-4" /> Danh sách
          </button>
        )}
      </h1>
      <p className="text-gray-500 text-sm mb-4">{submissions.length} học sinh đã nộp</p>

      <div className="md:flex md:gap-6">

        {/* Submission list — hidden on mobile when detail is open */}
        <div className={`${showDetail ? 'hidden md:block' : 'block'} md:w-72 md:flex-shrink-0`}>
                ...
                <button key={sub.id} onClick={() => loadDetail(sub.id)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className={`text-xs px-1.5 py-0.5 rounded-full ${STATUS_COLORS[sub.status]}`}>
                        {STATUS_LABELS[sub.status]}
                      </span>
                      {sub.score != null && <span className="text-xs text-green-600 font-medium">{sub.score}đ</span>}
                    </div>
                  </div>
        ...
        {/* Detail panel */}
        <div className={`${!showDetail ? 'hidden md:block' : 'block'} flex-1 mt-3 md:mt-0`}>
          {/* Mobile back button */}
          <button onClick={() => setShowDetail(false)} className="md:hidden flex items-center gap-1 text-blue-600 text-sm font-medium mb-3">
          ...
          {!detail ? (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex items-center justify-center h-48">
              <p className="text-gray-400 text-sm">Chọn bài nộp để xem chi tiết</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  {detail.submittedAt && (
                    <p className="text-xs text-gray-400">Nộp: {format(new Date(detail.submittedAt), 'dd/MM/yyyy HH:mm')}</p>
                  )}
                </div>
              ...
              {detail.answers?.length > 0 && (
                <div className="mb-5">
                  <p className="text-sm font-medium text-gray-700 mb-3">
                    Câu trả lời ({detail.answers.filter(a => a.isCorrect).length}/{detail.answers.length} đúng):
                  </p>
                  <div className="space-y-2">
                    {detail.answers.map((ans, i) => (
                      <div key={ans.id}
                        ...
                        <p className="font-medium text-gray-800 mb-0.5 text-xs">Câu {i + 1}: {ans.questionText}</p>
                        <p className={`text-xs ${ans.isCorrect ? 'text-green-700' : 'text-red-600'}`}>
              ...
              {detail.assignmentType !== 'quiz' && (
                <div className="border-t border-gray-100 pt-4 space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    <label className="text-sm text-gray-600 whitespace-nowrap">Điểm (tối đa {detail.totalPoints})</label>
