...
export default function CreateAssignmentModal({
  ...
  useEffect(() => {
    if (!isEdit || editData.type !== "quiz" || editData.questions?.length > 0)
      return;
    setLoadingQ(true);
    assignmentApi
      .getById(editData.id)
      .then(({ data }) => {
        if (data.questions?.length)
          setQuestions(toQuestionsData(data.questions));
      })
        ...
        <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              disabled={isEdit}
              className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:bg-gray-50 disabled:text-gray-400"
            >
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, totalPoints: parseFloat(e.target.value) })
                }
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  type="button"
                  onClick={addQuestion}
                  className="flex items-center gap-1 text-sm text-purple-600 font-medium bg-purple-50 hover:bg-purple-100 px-3 py-1.5 rounded-lg transition-colors"
                >
                  <PlusIcon className="w-4 h-4" /> Thêm câu
                </button>
              </div>
              ...
              {loadingQ ? (
                ...
                <div className="space-y-3">
                  {questions.map((q, qi) => (
                    <div
                      key={qi}
                      className="border border-gray-200 rounded-xl p-3 md:p-4 bg-gray-50/50"
                    >
                      <div className="flex items-start gap-2.5 mb-3">
                        <span className="w-6 h-6 md:w-7 md:h-7 bg-purple-100 text-purple-700 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">
                          {qi + 1}
                        </span>
                        <div className="flex-1">
                          ...
                          <div className="flex gap-2 flex-wrap">
                            <select
                              ...
                              className="border border-gray-200 rounded-lg px-3 py-1.5 text-xs focus:outline-none bg-white flex-1 min-w-0"
                            >
                            ...
                            <div className="flex items-center gap-1">
                              <input
                                ...
                                className="w-16 border border-gray-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none bg-white"
                              />
                      ...
                      {/* Options */}
                      <div className="ml-8 md:ml-10 space-y-2">
                              ...
                              <div key={oi} className="flex items-center gap-2">
                                {q.questionType === "single_choice" ? (
                                  <input
                                    ...
                                    className="text-purple-600 flex-shrink-0 w-4 h-4"
                                  />
                                ) : (
                                  <input
                                    type="checkbox"
                                    ...
                                    className="rounded text-purple-600 flex-shrink-0 w-4 h-4"
                                  />
                                )}
                                <input
                                  ...
                                  required
                                  className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-purple-500 bg-white"
                                />
                              </div>
                            ))}
                      </div>
          ...
          <div className="flex gap-3 pt-3 border-t border-gray-100">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`flex-1 sm:flex-none px-6 py-3 rounded-xl text-sm font-medium text-white disabled:opacity-50
